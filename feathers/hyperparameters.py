...
class Hyperparameters:
    ...
    def __init__(self, nr_configs) -> None:
        self.sample_hyperparams = lambda: {
           'learning_rate': 0.0025, # np.round(10 ** (np.random.uniform(-4, -2)), 4),
           'weight_decay': 3e-5, #np.round(10.0 ** np.random.uniform(low=-5.0, high=-3.0), 6),
           'momentum': 0.9, #np.round(np.random.uniform(low=0.6, high=1.0), 2),
           'dropout': 0.1, #np.round(np.random.uniform(low=0, high=0.2), 2),
