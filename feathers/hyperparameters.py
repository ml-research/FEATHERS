...
class Hyperparameters:
    ...
    def __init__(self, nr_configs) -> None:
        self.sample_hyperparams = lambda: {
           'learning_rate': np.round(10 ** (np.random.uniform(-4, -1)), 4),
           'weight_decay': np.round(10.0 ** np.random.uniform(low=-4.0, high=-1.0), 6),
           'momentum': np.round(np.random.uniform(low=0.6, high=1.0), 2),
           'dropout': np.round(np.random.uniform(low=0.1, high=0.4), 2),
